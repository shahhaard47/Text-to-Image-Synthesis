...
class Trainer(object):
    def __init__(self, type, dataset, split, lr, diter, vis_screen, save_path, l1_coef, l2_coef, pre_trained_gen, pre_trained_disc, batch_size, num_workers, epochs):
        ...
        if dataset == 'birds': self.dataset = Text2ImageDataset(config['birds_dataset_path'], split=split)
        elif dataset == 'flowers': self.dataset = Text2ImageDataset(config['flowers_dataset_path'], split=split)
        else:
    ...
    def _train_naive(self):
        ...
        for epoch in range(self.num_epochs):
            for sample in self.data_loader:
                ...
                g_loss = self.l1_coef * l1_loss(fake_images, right_images)
